...
This will be used to control the feeder.
...
1. A private `TalonFX` or `SparkMax` motor controller object, named `motor`, with an ID of 5.
   ...
   This will be used to control the feeder motor at CANBus ID 5 on bus ID 0.
...
1. A private `TalonFX` or `SparkMax` motor controller object, named `motor`, with an ID of 4.
   It is connected to a Systemcore CANBus with ID 0.
   This will be used to control the motor at CANBus ID 4 on bus ID 0.
...
Now that we have the mechanism classes created, we need to set them up in the `Robot` class.
