...
to 0.9 forever. 3. A `intake()` command that sets throttle to 0.8 forever. 4. A
`outtake()` command that sets throttle to -0.8 forever. 5. A `idle()` command
that sets throttle to 0 forever. Make this the default command. 6. The code
needed for simulating the intake launcher. You haven't officially learned how to
do this, but task 2's final section gives a hint. Can you figure it out?
