---
title: Drivetrain Motors
description: Adding motors for the kitbot drivetrain
prev: stage-overview
next: false
---
...
Arcade drive uses a single joystick to control the direction of travel and rotation.
...
When created the `DifferentialDrive` is asking for a `DoubleConsumer` for the left and right motors.
...
When given a joystick input, the `DifferentialDrive` instance will call the functions provided to it automatically commanding the motors to move to produce the desired motion.
...
Therefore, when provided with input from the joysticks, the `DifferentialDrive` instance will call the provided motors controller's `setThrottle()` functions with the correct duty cycle to produce the commanded motion.

<Aside>
The `DifferentialDrive` instance was only provided with the Leader motor controller's `setThrottle()` functions since the Follower motors will automatically follow.
</Aside>