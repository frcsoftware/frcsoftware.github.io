...
next: false
codeRegionSources:
    default: stage0/snippets/JavaFundamentals.java
---
...
- String: holds a sequence of characters. Denoted by double quotes (").
  Example: “Hello World”
- char: holds a Single character. Denoted by single quotes (').
  Example: 'A'
...
One major exception is spaces, which are forbidden in variable names. Instead, you should write variables in what's known as camel case (frontLeftDrive).
Upper snake case (FRONT_LEFT_DRIVE) should be used instead for constants, which we will learn about later in the course.
Variable names can have numbers at any point except for the first character. (motor2 is legal, 2motor is not).
