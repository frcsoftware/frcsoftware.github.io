...
next: false
codeRegionSources:
    point: stage0/snippets/classes-methods/Point.java
    tracker: stage0/snippets/classes-methods/RobotTracker.java
    usage: stage0/snippets/classes-methods/Usage.java
---

import Aside from '@components/Aside.astro';

In an [earlier lesson](../java-fundamentals/), we discussed different data types in Java,
...
They can also return a value, which is the result of the method's computation.
...
The constructor is a special method called when an object is created with the `new` keyword.
...
Getter methods are methods that simply return the value of a field.
...
we then pass `this.x + other.x` and `this.y + other.y` as the parameters to the constructor,
so the new `Point` object will have the sum of the two `Point` objects' coordinates.
...
The `move` method accepts a `delta`, the change in position, and adds it to the current position.
